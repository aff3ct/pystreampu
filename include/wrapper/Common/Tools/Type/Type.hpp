...
{
    py::class_<pyspu::dtype>(m,
                            "dtype",
                            py::dynamic_attr(),
                            R"mydelimiter(
StreamPU abstract class for data type object.)mydelimiter")
      .def_property_readonly("name", &pyspu::dtype::get_name, "dtype's name")
      .def_property_readonly(
        "is_signed", &pyspu::dtype::is_signed, "True if dtype names a signed type, False otherwise.")
      .def_property_readonly("is_floating_point",
                             &pyspu::dtype::is_floating_point,
                             "True if dtype names a floating point type, False otherwise.")
      .def_property_readonly("size", &pyspu::dtype::get_size, "Byte size of the dtype.")
      .def_property_readonly(
        "numpy",
        [](const pyspu::dtype& slf)
        { return py::module_::import("numpy").attr("dtype")(slf.get_name().c_str()).attr("type"); },
        "Get the corresponding numpy dtype.")
      .def_static("of", &pyspu::dtype::get, "name"_a, "Dtype factory.")
      .def("__repr__", [](const pyspu::dtype& slf) { return "spu." + slf.get_name(); })
      .def("__str__", [](const pyspu::dtype& slf) { return slf.get_name(); })
      .doc();
}
...
wrap_dtypes(py::module_& m)
{
    ...
    full_lT::for_each(
      [&](auto t)
      {
          ...
          m.attr(spu::runtime::type_to_string[typeid(T)].c_str()) = py::cast(pyspu::dtype::of<T>());
      });
}
} // namespace wrapper
}; // namespace pyspu
