...
#define BOP_WRAP(name, TIlist, TOlist)\
	...
	lTI::for_each([&](auto ti) {\
		...
		lTO::for_each([&](auto to) {\
			...
			pyaf::wrapper::internal_wrap_binaryop<TI,TO,aff3ct::tools::bop_##name<TI,TO>>(scope, "Binaryop_" + std::string(#name) + "_" + TI_str + "_" + TO_str);\
		});\
	});
...
void internal_wrap_binaryop(py::handle scope, const std::string& name)
{
    ...
    auto py_bop_class = py::class_<aff3ct::module::Binaryop<TI,TO,BOP>, aff3ct::module::Module>(scope, name.c_str());
    py_bop_class.def(py::init<const size_t,const size_t>(),"n_in1"_a,"n_in2"_a, doc_str, py::return_value_policy::take_ownership);
