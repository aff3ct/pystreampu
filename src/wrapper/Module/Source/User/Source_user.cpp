...
void pyaf::wrapper::wrap_source_user(py::handle scope) {
  ...
  lB::for_each([&](auto b) {
    ...
    auto source_class =
        py::class_<spu::module::Source_user<B>, spu::module::Source<B>>(
            scope, std::string("Source_user_" + B_str).c_str());
    source_class.def(
        py::init<const int, const std::string &, const bool, const int>(),
        "max_data_size"_a, "filename"_a, "auto_reset"_a = true,
        "start_idx"_a = 0, py::return_value_policy::take_ownership);
  });
}