...
std::map<std::type_index, std::string> type_map = {
    {typeid(int8_t), py::format_descriptor<int8_t>::format()},
...
void pyaf::wrapper::wrap_socket(py::handle scope) {
  py::class_<Socket, spu::tools::Interface_reset,
             spu::runtime::Socket_Publicist>
      py_socket(scope, "Socket", py::buffer_protocol(), py::dynamic_attr());

  py::enum_<socket_t>(py_socket, "directions",
                      "Enumeration of socket directions")
  ...
  py_socket.def_buffer([](Socket &s) -> py::buffer_info {
    if (s.get_dataptr() == nullptr)
      return py::buffer_info(nullptr, s.get_datatype_size(),
                             type_map[s.get_datatype()], 0);
  ...
  py_socket.def_property_readonly(
      "wave", [](const spu::runtime::Socket &self) -> py::array {
        if (self.get_dataptr() == nullptr)
          return py::array(
              py::buffer_info(nullptr, 1, type_map[self.get_datatype()], {}));
        ...
        return py::array(
            py::buffer_info(
                ...
                2,                             /* Number of dimensions */
                {n_row, n_col},                /* Buffer dimensions */
                {(size_t)self.get_datatype_size() * n_col,
                 (size_t)self.get_datatype_size()},
                self.get_type() == socket_t::SIN),
            buffer_handle);
      });
  py_socket.def_property_readonly("task", &spu::runtime::Socket::get_task,
                                  "Task owning the socket.");
  py_socket.def_property_readonly("n_elmts", &spu::runtime::Socket::get_n_elmts,
                                  "Number of elements per `n_frames`");
  py_socket.def_property_readonly(
      "bound_sockets", &spu::runtime::Socket::get_bound_sockets,
      "Sockets to wich the socket is bound (for output sockets only).");
  py_socket.def_property_readonly(
      "dtype",
      [](const spu::runtime::Socket &self) {
        return pyaf::dtype::get(self.get_datatype_string());
      },
      "Data type.");
  py_socket.def_property_readonly(
      "bound_socket",
      static_cast<Socket &(Socket::*)()>(
          &spu::runtime::Socket::get_bound_socket),
  ...
  py_socket.def(
      "_bind",
      [](spu::runtime::Socket &self, spu::runtime::Socket &s_out,
         const int priority) { self.bind(s_out, priority); },
      "Binds the socket to socket 's_out' with priority 'priority'.", "s_out"_a,
      "priority"_a = -1);
  ...
  py_socket.def(
      ...
      [](spu::runtime::Socket &self, py::array &arr) {
        ...
        if (n_row > 1 && buffer.ndim == 2) {
          if ((size_t)buffer.shape[0] != n_row ||
              (size_t)buffer.shape[1] != n_col) {
            std::stringstream message;
            message << "The shape of the array must match the socket one.";
            message << "Socket shape: " << n_row << " x " << n_col << ".";
            message << "Array shape: " << buffer.shape[0] << " x "
                    << buffer.shape[1] << ".\n";
            throw std::runtime_error(message.str());
          }
        ...
        if (!arr.dtype().is(py_self.dtype())) {
          ...
          message << "Socket dtype: "
                  << py_self.dtype().attr("name").cast<std::string>() << ".\n";
          message << "Array dtype: "
                  << arr.dtype().attr("name").cast<std::string>() << ".\n";
          throw std::runtime_error(message.str());
        }
  ...
  py_socket.def_property_readonly("direction", &spu::runtime::Socket::get_type,
