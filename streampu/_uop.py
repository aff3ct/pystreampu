...
def _uop_factory(n_elmts: int, uop_type: UType, input_dtype: dtype = int8, output_dtype: dtype = int8) -> Module:
    ...
    Returns:
        Module: StreamPU module unary operator

    Raises:
        TypeError: if input_type or output_type are not dtypes
    """
    if not isinstance(input_dtype, dtype):
        msg = f"type '{type(input_dtype)}' does not name an StreamPU datatype."
        raise TypeError(msg)
    ...
    uop_class = getattr(streampu._ext.uop, name)
...
def uop(uop_type: UType, sckt: Socket, output_dtype: dtype = None) -> Socket:
    ...
    _context_manager.store_task(mdl.perform)
