...
#define RED_WRAP(name, TIlist, TOlist)                                         \
  ...
  lTI::for_each([&](auto ti) {                                                 \
    ...
    lTO::for_each([&](auto to) {                                               \
      ...
      pyaf::wrapper::internal_wrap_reducer<TI, TO,                             \
                                           spu::tools::bop_##name<TI, TO>>(    \
          scope,                                                               \
          "Reducer_" + std::string(#name) + "_" + TI_str + "_" + TO_str);      \
    });                                                                        \
  });
...
void internal_wrap_reducer(py::handle scope, const std::string &name) {
  ...
  auto py_red_class =
      py::class_<spu::module::Reducer<TI, TO, BOP>, spu::module::Module>(
          scope, name.c_str());
  py_red_class.def(py::init<const size_t>(), "n_elmts"_a, doc_str,
