...
def test_simple_pipeline(n_threads: int,
    ...
    test_result = simple_pipeline(n_threads, n_inter_frames, sleep_time_us,
                                  data_length, buffer_size, dot_filepath,
                                  in_filepath, out_filepath, copy_mode,
                                  print_stats, step_by_step, debug,
                                  force_sequence, active_waiting)
...
def simple_pipeline(n_threads: int = HW_CONCURRENCY-2,
    ...
    source = aff3ct.source_user_binary(data_length,
                                       in_filepath,
                                       auto_reset=False,
                                       dtype=aff3ct.uint8)

    sink = aff3ct.sink_user_binary(data_length,
                                   out_filepath,
                                   dtype=aff3ct.uint8)
    ...
    theoretical_time = n_frames * (len(rlys) * sleep_time_us * 1000) * n_inter_frames / 1000000.0 / n_threads
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test a simple AFF3CT chain.',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('-t', '--n-threads',      type = int, dest = 'n_threads',      default = max_threads-2,
                        help = 'Number of threads to run in parallel')
    parser.add_argument('-f', '--n-inter-frames', type = int, dest = 'n_inter_frames', default = 1,
                        help = 'Number of frames to process in one task')
    parser.add_argument('-s', '--sleep-time',     type = int, dest = 'sleep_time_us',  default = 5,
                        help = 'Sleep time duration in one task (microseconds)')
    parser.add_argument('-d', '--data-length',    type = int, dest = 'data_length',    default = 2048,
                        help = 'Size of data to process in one task (in bytes)')
    parser.add_argument('-u', '--buffer-size',    type = int, dest = 'buffer_size',    default = 16,
                        help = 'Size of the buffer between the different stages of the pipeline')
    parser.add_argument('-o', '--dot-filepath',   type = str, dest = 'dot_filepath',   default = '',
                        help = 'Path to dot output file')
    parser.add_argument('-i', '--in-filepath',    type = str, dest = 'in_filepath',    default = '',
                        help = 'Path to dot input file')
    parser.add_argument('-j', '--out-filepath',   type = str, dest = 'out_filepath',   default = 'file.out',
                        help = 'Path to dot input file')
    parser.add_argument('-c', '--copy-mode',     action='store_true', dest = 'copy_mode',     default = False,
                        help = 'Enable to copy data in sequence (performance will be reduced)')
    parser.add_argument('-p', '--print-stats',   action='store_true', dest = 'print_stats',   default = False,
                        help = 'Enable to print per task statistics (performance will be reduced)')
    parser.add_argument('-b', '--step-by-step',  action='store_true', dest = 'step_by_step',  default = False,
    ...
    parser.add_argument('-q', '--force-sequence',action='store_true', dest = 'force_sequence',default = False,
                        help = 'Force sequence instead of pipeline')
    parser.add_argument('-w', '--active_wating', action='store_true', dest = 'active_waiting',default = False,
