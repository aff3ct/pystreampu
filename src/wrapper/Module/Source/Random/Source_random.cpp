...
void pyaf::wrapper::wrap_source_random(py::handle scope) {
  ...
  lB::for_each([&](auto b) {
    ...
    auto source_class =
        py::class_<spu::module::Source_random<B>, spu::module::Source<B>>(
            scope, std::string("Source_random_" + B_str).c_str());
    source_class.def(py::init<const int, const int>(), "max_data_size"_a,
                     "seed"_a = 0, py::return_value_policy::take_ownership);
  });
}
