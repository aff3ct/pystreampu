...
void pyaf::wrapper::wrap_source_user_binary(py::handle scope) {
  ...
  lB::for_each([&](auto b) {
    ...
    auto source_class =
        py::class_<spu::module::Source_user_binary<B>, spu::module::Source<B>>(
            scope, std::string("Source_user_binary_" + B_str).c_str());
    source_class.def(
        py::init<const int, const std::string &, const bool, const bool>(),
        "max_data_size"_a, "filename"_a, "auto_reset"_a = true,
        "fifo_mode"_a = false, py::return_value_policy::take_ownership);
  });
}