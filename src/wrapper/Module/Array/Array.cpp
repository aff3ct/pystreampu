...
void pyaf::wrapper::wrap_array(py::module_ &scope) {
  ...
  lTI::for_each([&](auto ti) {
    ...
    auto arra_py_class =
        py::class_<spu::module::Array<TI>, spu::module::Module>(
            scope, std::string("Array_" + TI_str).c_str());

    arra_py_class.def(py::init<py::array_t<TI> &>(), "data"_a,
                      py::keep_alive<1, 2>(), R"pbdoc()pbdoc",
...
template <typename T>
Array<T>::Array(py::array_t<T> &data) : Module(), dataptr(data.request().ptr) {
  ...
  this->create_codelet(
      p, [](Module &m, runtime::Task &t, const size_t frame_id) -> int {
        return runtime::status_t::SUCCESS;
      });
}

template <typename T> void *Array<T>::get_dataptr() const {
  return this->dataptr;
}
