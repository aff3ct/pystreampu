...
void pyaf::wrapper::wrap_sink_no(py::handle scope) {
  ...
  lB::for_each([&](auto b) {
    ...
    auto sink_class = py::class_<spu::module::Sink_NO<B>, spu::module::Sink<B>>(
        scope, std::string("Sink_NO_" + B_str).c_str());
    sink_class.def(py::init<const int>(), "max_data_size"_a,
