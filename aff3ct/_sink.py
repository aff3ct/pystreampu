...
def sink(max_data_size: int, dtype: aff3ct._ext.dtype = int16):
...
def sink_no(max_data_size: int, dtype: aff3ct._ext.dtype = int16):
    if not isinstance(dtype, aff3ct._ext.dtype):
        msg = f"type '{type(dtype)}' does not name an AFF3CT datatype."
        raise TypeError(msg)
...
def sink_user_binary(
    max_data_size: int, filename: str, dtype: aff3ct._ext.dtype = int16
):
    if not isinstance(dtype, aff3ct._ext.dtype):
        msg = f"type '{type(dtype)}' does not name an AFF3CT datatype."
        raise TypeError(msg)
