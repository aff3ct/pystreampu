...
namespace spu {
namespace module {
template <typename T> void Slicer ::_task_init() {

  auto &p = this->create_task("write");
  size_t ps_x = this->template create_socket_fwd<T>(p, "x", this->n_elmts);
  size_t ps_nx =
      this->template create_socket_in<T>(p, "nx", this->slice.size());

  this->create_codelet(p,
                       [ps_x, ps_nx](spu::module::Module &m,
                                     spu::runtime::Task &t,
  ...
  this->create_codelet(p1,
                       [p1s_x, p1s_nx](spu::module::Module &m,
                                       spu::runtime::Task &t,
                                       const size_t frame_id) -> int {
                         auto &slicer = static_cast<Slicer &>(m);
  ...
  size_t p2s_xs =
      this->template create_socket_out<T>(p2, "xs", this->slice.size());

  this->create_codelet(p2,
                       [p2s_x, p2s_xs](spu::module::Module &m,
                                       spu::runtime::Task &t,
                                       const size_t frame_id) -> int {
                         auto &slicer = static_cast<Slicer &>(m);
