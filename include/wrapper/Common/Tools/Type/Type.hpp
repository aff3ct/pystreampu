...
protected:
  explicit dtype(const std::string &name, const int size, const bool _signed,
                 const bool _floating_point)
      : name(name), size(size), _signed(_signed),
...
public:
  ...
  template <typename T = float> static pyaf::dtype of() {
    std::string name = spu::runtime::type_to_string[typeid(T)].c_str();
    return pyaf::dtype(name, sizeof(T), std::is_signed<T>::value,
...
inline void wrap_dtypes(py::module_ &m) {
  ...
  full_lT::for_each([&](auto t) {
    ...
    m.attr(spu::runtime::type_to_string[typeid(T)].c_str()) =
        py::cast(pyaf::dtype::of<T>());
  });
}
} // namespace wrapper
}; // namespace pyaf
