...
void pyaf::wrapper::wrap_probe(py::handle scope)
{
	...
	lT::for_each([&](auto t){
		...
		//probe_class.def(py::init<const int, const std::string &>(), "size"_a, "col_name"_a, py::return_value_policy::take_ownership);
		...
		py::class_<spu::module::Probe_value<T>, spu::module::Probe<T>>(scope, std::string("Probe_value_" + T_str).c_str())
			.def(py::init<const int, const std::string &, spu::tools::Reporter_probe*>(), "size"_a, "col_name"_a, "reporter"_a, py::keep_alive<1, 2>(), py::return_value_policy::take_ownership);
	});
