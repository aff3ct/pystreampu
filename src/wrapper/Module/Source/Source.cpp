...
void pyaf::wrapper::wrap_source(py::handle scope) {
  ...
  lB::for_each([&](auto b) {
    ...
    auto source_class =
        py::class_<spu::module::Source<B>, spu::module::Module,
                   spu::tools::Interface_set_seed,
                   spu::tools::Interface_is_done, spu::tools::Interface_reset>(
            scope, std::string("Source_" + B_str).c_str());
    source_class.def(py::init<const int>(), "max_data_size"_a,
