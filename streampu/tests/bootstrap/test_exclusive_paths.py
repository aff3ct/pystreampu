...
def exclusive_paths(
    ...
    if path >= 3:
        raise streampu.exceptions.InvalidArgument("'path' has to be smaller than 3")

    controller = None
    if cyclic_path:
        controller = streampu.Controller_cyclic(3, 0)
    else:
        controller = streampu.Controller_static(path)

    switcher = streampu.Switcher(3, data_length, streampu.uint8)
    ...
    sequence_chain = streampu.Sequence(data.task, n_threads)
