...
protected:
    ...
    :name(name), size(size), _signed(_signed), _floating_point(_floating_point)
    {};
...
inline void wrap_generic_dtype(py::module_ &m)
{
    py::class_<pyaf::dtype>(m, "dtype")
        ...
        .def                  ("__repr__", [](const pyaf::dtype& slf){return "pyaf." + slf.get_name();});
}
