...
template <typename type_list, unsigned int index = 0, bool stop = ( index == type_list_length<type_list>::value )> struct type_switch
{
void operator()( unsigned int i)
{
    if( i == index )
    return f.template operator()<typename type_listGet<type_list, index>::type>();
    else
    {
