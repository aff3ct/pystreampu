...
void pyaf::wrapper::wrap_probe(py::handle scope) {
  ...
  auto aprobe_class =
      py::class_<spu::module::AProbe, spu::module::Module,
                 spu::tools::Interface_reset,
                 std::unique_ptr<spu::module::AProbe, py::nodelete>>(scope,
                                                                     "AProbe");
  ...
  lT::for_each([&](auto t) {
    ...
    auto probe_class = py::class_<spu::module::Probe<T>, spu::module::AProbe,
                                  pyaf::wrapper::py_Probe<T>>(
    ...
    probe_class.def("set_col_buff_size",
                    &spu::module::Probe<T>::set_col_buff_size);
    probe_class.def("set_col_fmtflags",
                    &spu::module::Probe<T>::set_col_fmtflags);
    probe_class.def("set_col_prec", &spu::module::Probe<T>::set_col_prec);
    probe_class.def("set_col_size", &spu::module::Probe<T>::set_col_size);
    probe_class.def("register_reporter",
                    &spu::module::Probe<T>::register_reporter,
                    py::keep_alive<1, 2>());
    probe_class.def("get_col_name", &spu::module::Probe<T>::get_col_name);

    py::class_<spu::module::Probe_value<T>, spu::module::Probe<T>>(
        scope, std::string("Probe_value_" + T_str).c_str())
        .def(py::init<const int, const std::string &,
                      spu::tools::Reporter_probe *>(),
             "size"_a, "col_name"_a, "reporter"_a, py::keep_alive<1, 2>(),
