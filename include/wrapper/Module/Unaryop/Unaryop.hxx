...
#define UOP_WRAP(name, types)                                                  \
  ...
  int_lT::for_each([&](auto ti) {                                              \
    ...
    int_lT::for_each([&](auto to) {                                            \
      ...
      pyaf::wrapper::internal_wrap_unaryop<TI, TO,                             \
                                           spu::tools::uop_##name<TI, TO>>(    \
          scope,                                                               \
          "Unaryop_" + std::string(#name) + "_" + TI_str + "_" + TO_str);      \
    });                                                                        \
  });
...
void internal_wrap_unaryop(py::handle scope, const std::string &name) {
  ...
  auto py_uop_class =
      py::class_<spu::module::Unaryop<TI, TO, UOP>, spu::module::Module>(
          scope, name.c_str());
  py_uop_class.def(py::init<const size_t>(), "n_elmts"_a, doc_str,
