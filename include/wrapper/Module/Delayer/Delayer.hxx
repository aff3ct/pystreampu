...
void wrap_delayer_internal(py::handle scope, const std::string& name)
{
    ...
    [](spu::module::Delayer<D> self, const std::vector<D>& data)
    {
        self.set_data(data);
    });
    py_delayer_class.def("set_data",
    [](spu::module::Delayer<D> self, const D* data)
    {
        self.set_data(data);
    });
    py_delayer_class.def_property("n_frames", &spu::module::Delayer<D>::get_n_frames, &spu::module::Delayer<D>::set_n_frames);
}
