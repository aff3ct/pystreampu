...
void wrap_initializer_internal(py::handle scope, const std::string& name)
{
    ...
    //py_initializer_class.def("set_data", [](spu::module::Initializer<T> self, const std::vector<std::vector<T>>& data){
    //    self.set_init_data(data);});
    //py_initializer_class.def("set_data", [](spu::module::Initializer<T> self, const std::vector<T>& data)             {
    //    self.set_init_data(data);},py::keep_alive<1, 2>());
    py_initializer_class.def_property_readonly("data_init", [](const spu::module::Initializer<T>& self){
        const std::vector<std::vector<T>>& init_data = self.get_init_data();
        py::list lst_out;
        for (auto & v : init_data)
            lst_out.append(py::array{(ssize_t)v.size(), (T*)v.data(), py::cast(self)});
        return lst_out;
    });
    py_initializer_class.def_property("n_frames", &spu::module::Initializer<T>::get_n_frames, &spu::module::Initializer<T>::set_n_frames);
}
